...
cd "${SRC_ROOT}"/"${PKGNAME}"/myricom && sed -i -s 's:mkdir\ \-p\ \$installroot::' configure && ./configure --with-myricom=/opt/snf --bro-dist=/home/mpurzynski/bro --install-root=/opt/bro/lib/bro/plugins || exit 1
make || exit 1
